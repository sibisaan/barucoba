...
def get_initial_data():
    try:
        ...
        logger.info(f"GET response status code: {response.status_code}")  # Menampilkan status code
        logger.info(f"GET response text: {response.text}")  # Menampilkan isi respons
        if response.status_code == 200:
            ...
            return response.text  # Return the body of the response in case of failure
...
def post_login_request(user_token, email, md5_password):
    ...
    try:
        ...
        logger.info(f"POST response status code: {response.status_code}")  # Menampilkan status code
        logger.info(f"POST response text: {response.text}")  # Menampilkan isi respons
        if response.status_code == 200:
...
async def handle_password(update: Update, context: CallbackContext):
    ...
    if user_token:
        ...
        # Kirim respons login ke pengguna
        if login_response:
            await update.message.reply_text(f"Login Response: {login_response}")
        else:
